...
3. The `TEMPLATE_SecondStage.ini` file that you downloaded and copied already includes a set of fundamental climate variables and the bare minimum flux variables to continue on and run the third stage. We advise testing the file with only these variables first. Note that even if you do not change the content of any traces during the second stage, you must still include any trace that you wish to be available for the third stage (and beyond, e.g., Ameriflux submission), i.e., they must be passed along the pipeline.
4. Test the second stage data cleaning in Matlab, as follows:
    <pre>
    fr_automated_cleaning(yearIn,'SITEID',2)</pre>
...
<!-- [add fluxes to second stage template! needed for third stage example] -->
