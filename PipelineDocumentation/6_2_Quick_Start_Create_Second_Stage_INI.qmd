...
3. The `TEMPLATE_SecondStage.ini` file provided already includes a set of fundamental climate variables and the bare minimum flux variables to continue on and run the third stage. We advise testing the file with only these variables first. Note that even if you do not change the content of any traces during the second stage, you must still include any trace that you wish to be available for the third stage (and beyond, e.g., Ameriflux submission), i.e., they must be passed along the pipeline.
